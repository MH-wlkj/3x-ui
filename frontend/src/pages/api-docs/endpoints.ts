...
export const sections: readonly Section[] = [
  ...
  {
    id: 'portal',
    title: 'Tenant Portal (User Management)',
    description:
      'Admin tenant-user management lives under /panel/api/portal/users and uses the same session/Bearer auth as the rest of /panel/api. The tenant-facing portal API lives under /portal/api and authenticates with a bearer token returned by POST /portal/api/login. Each tenant can only create and manage clients on the inbounds an admin assigned to them, and is capped at a total number of clients.',
    endpoints: [
      {
        method: 'GET',
      ...
      {
        method: 'POST',
        path: '/panel/api/portal/users/create',
        summary: 'Create a tenant account with allowed inbounds and a client limit.',
        params: [
          { name: 'username', in: 'body (json)', type: 'string', desc: 'Tenant login name.' },
      ...
      {
        method: 'POST',
        path: '/panel/api/portal/users/update',
        summary: 'Update a tenant account (username/password/inbounds/limit). A blank username or password leaves that value unchanged.',
        params: [
          { name: 'id', in: 'body (json)', type: 'integer', desc: 'Tenant user id.' },
          { name: 'username', in: 'body (json)', type: 'string', desc: 'New login name (blank = keep).', optional: true },
      ...
      {
        method: 'POST',
        path: '/panel/api/portal/users/delete',
        summary: 'Delete a tenant account.',
        params: [{ name: 'id', in: 'body (json)', type: 'integer', desc: 'Tenant user id.' }],
        body: '{\n  "id": 1\n}',
        response: '{\n  "success": true,\n  "msg": "user deleted"\n}',
      },
      {
        ...
        summary: 'Tenant login. Returns a bearer token valid for 24 hours.',
        params: [
          { name: 'username', in: 'body (json)', type: 'string', desc: 'Tenant username.' },
          { name: 'password', in: 'body (json)', type: 'string', desc: 'Tenant password.' },
        ],
        body: '{\n  "username": "reseller1",\n  "password": "secret"\n}',
        response: '{\n  "success": true,\n  "obj": { "token": "..." }\n}',
      },
      ...
      },
      {
        method: 'GET',
        path: '/portal/api/me',
        summary: 'Tenant quota view: allowed inbounds, client limit and used count. Requires Bearer token.',
        responseSchema: 'UserStatus',
      },
      {
        method: 'GET',
        path: '/portal/api/inbounds',
        summary: 'Inbound options the tenant is allowed to target. Requires Bearer token.',
        responseSchemaArray: true,
        responseSchema: 'InboundOption',
      },
      {
        ...
        summary: 'Create one client on an allowed inbound (enforces inbound permission and client quota). Requires Bearer token.',
        params: [
          { name: 'inboundId', in: 'body (json)', type: 'integer', desc: 'Target inbound id (must be assigned to the tenant).' },
          { name: 'email', in: 'body (json)', type: 'string', desc: 'Client email.' },
          { name: 'totalGB', in: 'body (json)', type: 'number', desc: 'Traffic limit in bytes. 0 = unlimited.', optional: true },
          { name: 'expiryTime', in: 'body (json)', type: 'number', desc: 'Expiry unix-ms timestamp. 0 = never expires.', optional: true },
        ],
        body: '{\n  "inboundId": 1,\n  "email": "user-001",\n  "totalGB": 0,\n  "expiryTime": 0\n}',
      },
      {
      ...
      {
        method: 'GET',
        path: '/portal/api/clients/node/:email',
        summary: 'Outbound node target (address/port/creds) for one of the tenant\u2019s clients. Requires Bearer token.',
        params: [{ name: 'email', in: 'path', type: 'string', desc: 'Client email (must belong to the tenant).' }],
        responseSchema: 'PortalNodeView',
      },
      {
        method: 'POST',
        path: '/portal/api/clients/update',
        summary: 'Edit one of the tenant\u2019s own clients (email, traffic, expiry, enable, flow). Requires Bearer token.',
        params: [
          { name: 'email', in: 'body (json)', type: 'string', desc: 'Current client email.' },
          { name: 'client', in: 'body (json)', type: 'object', desc: 'Client fields to update: email, totalGB (bytes), expiryTime (ms), enable, flow.' },
        ],
        body: '{\n  "email": "user-001",\n  "client": { "email": "user-001", "totalGB": 0, "expiryTime": 0, "enable": true }\n}',
      },
      {
        method: 'POST',
        path: '/portal/api/xray/node',
        summary: 'Update the outbound node target (address/port/creds) for one of the tenant\u2019s clients. The routing rule keeps pointing the client email to the same outbound tag. Changes the shared panel Xray config. Requires Bearer token.',
        params: [
          { name: 'email', in: 'body (json)', type: 'string', desc: 'Client email (must belong to the tenant).' },
          { name: 'address', in: 'body (json)', type: 'string', desc: 'New upstream node address.' },
          { name: 'port', in: 'body (json)', type: 'integer', desc: 'New upstream node port.' },
          { name: 'user', in: 'body (json)', type: 'string', desc: 'New username (blank = keep).', optional: true },
          { name: 'pass', in: 'body (json)', type: 'string', desc: 'New password (blank = keep).', optional: true },
        ],
        body: '{\n  "email": "user-001",\n  "address": "198.65.65.250",\n  "port": 7176,\n  "user": "u",\n  "pass": "p"\n}',
      },
      {
        method: 'POST',
        path: '/portal/api/clients/delete',
      ...
      {
        method: 'POST',
        path: '/portal/api/clients/bulk-delete',
        summary: 'Delete several of the tenant\u2019s own clients by email (non-owned emails are skipped). Requires Bearer token.',
        params: [{ name: 'emails', in: 'body (json)', type: 'array', desc: 'Client emails to delete.' }],
        body: '{\n  "emails": ["user-001", "user-002"]\n}',
        response: '{\n  "success": true,\n  "obj": { "deleted": 2 }\n}',
      },
      {
        method: 'POST',
      ...
      {
        method: 'POST',
        path: '/portal/api/password',
        summary: 'Change the tenant\u2019s own password. Requires Bearer token.',
        params: [
          { name: 'oldPassword', in: 'body (json)', type: 'string', desc: 'Current password.' },
          { name: 'newPassword', in: 'body (json)', type: 'string', desc: 'New password.' },
        ],
